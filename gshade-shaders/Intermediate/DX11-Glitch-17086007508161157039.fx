...
{
return tex2D(samp, uv - fmod(uv, float2(pixelSize,pixelSize) /  float2(2560, 1440)));
}
#line 68
float rand(float n)
{
...
{
const float2 pixel =  float2((1.0 / 2560), (1.0 / 1440));
...
float Amount;
#line 126
const float2 texcoord = fragCoord *  float2(2560, 1440); 
const float2 uv = texcoord.xy /  float2(2560, 1440);
if (bUseUV) {
Amount = uv.x; 
}
