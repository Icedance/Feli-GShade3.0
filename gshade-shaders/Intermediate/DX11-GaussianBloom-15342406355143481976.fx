...
#line 67
texture GaussianBloomTex { Width = 1024* 0.5; Height = 720* 0.5; Format = RGBA8; };
texture GaussianBloomTex2 { Width = 1024* 0.5; Height = 720* 0.5; Format = RGBA8; };
#line 70
...
for(int i = 1; i < 4; ++i)
{
blur += tex2D(GaussianBloomSampler, texcoord + float2(0.0, offset[i] *  float2((1.0 / 1024), (1.0 / 720)).y)).rgb * weight[i];
blur += tex2D(GaussianBloomSampler, texcoord - float2(0.0, offset[i] *  float2((1.0 / 1024), (1.0 / 720)).y)).rgb * weight[i];
}
}
#line 131
...
for(int i = 1; i < 6; ++i)
{
blur += tex2D(GaussianBloomSampler2, texcoord + float2( offset[i] *  float2((1.0 / 1024), (1.0 / 720)).x,0.0)).rgb * weight[i];
blur += tex2D(GaussianBloomSampler2, texcoord - float2( offset[i] *  float2((1.0 / 1024), (1.0 / 720)).x,0.0)).rgb * weight[i];
}
}
#line 214
