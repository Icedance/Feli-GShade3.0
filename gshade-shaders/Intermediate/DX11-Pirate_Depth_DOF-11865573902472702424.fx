...
#line 129
texture2D	TexNormalDepth {Width = 1024 * 		0.5	; Height = 720 * 		0.5	; Format = RGBA16; MipLevels = 		5	;};
...
vec.y -= vec.y * (1.0 - vec.z) * sin(radians(FOV * (  	float2((1.0 / 1024), (1.0 / 720)).y /   	float2((1.0 / 1024), (1.0 / 720)).x)));
return vec;
}
#line 163
float4 GetNormalDepth(float2 coords)
{
const float2 offsety = float2(0.0,   	float2((1.0 / 1024), (1.0 / 720)).y);
const float2 offsetx = float2(  	float2((1.0 / 1024), (1.0 / 720)).x, 0.0);
#line 168
...
if (discradius <   	float2((1.0 / 1024), (1.0 / 720)).x / 		0.5	)
return origcolor;
const float2 calcv = v * discradius *   	float2((1.0 / 1024), (1.0 / 720)) / 		0.5	;
#line 300
for(int i=1; i <= 			4	; i++)
{
#line 304
