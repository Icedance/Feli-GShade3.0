#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\GaussianBloom.fx"
#line 11
...
#line 65 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\GaussianBloom.fx"
#line 67
texture GaussianBloomTex { Width = 2560* 0.5; Height = 1377* 0.5; Format = RGBA8; };
texture GaussianBloomTex2 { Width = 2560* 0.5; Height = 1377* 0.5; Format = RGBA8; };
#line 70
...
for(int i = 1; i < 4; ++i)
{
blur += tex2D(GaussianBloomSampler, texcoord + float2(0.0, offset[i] *  float2((1.0 / 2560), (1.0 / 1377)).y)).rgb * weight[i];
blur += tex2D(GaussianBloomSampler, texcoord - float2(0.0, offset[i] *  float2((1.0 / 2560), (1.0 / 1377)).y)).rgb * weight[i];
}
}
#line 131
...
for(int i = 1; i < 6; ++i)
{
blur += tex2D(GaussianBloomSampler2, texcoord + float2( offset[i] *  float2((1.0 / 2560), (1.0 / 1377)).x,0.0)).rgb * weight[i];
blur += tex2D(GaussianBloomSampler2, texcoord - float2( offset[i] *  float2((1.0 / 2560), (1.0 / 1377)).x,0.0)).rgb * weight[i];
}
}
#line 214
