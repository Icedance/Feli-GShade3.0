...
#line 447
texture2D VHS_InputTexA    { Width = 1024; Height = 720; Format = RGBA8; };
texture2D VHS_InputTexB    { Width = 1024; Height = 720; Format = RGBA8; };
texture2D VHS_FeedbackTexA { Width = 1024; Height = 720; Format = RGBA8; };
texture2D VHS_FeedbackTexB { Width = 1024; Height = 720; Format = RGBA8; };
texture2D _TapeTex { Width =  	int(((float)   int( 240.0	)*(float)1024/(float)720)); Height =    int( 240.0	); Format = RGBA8; };
#line 453
...
#line 707
const float y = p.y* float2(1024, 720).y;
const float s = t*2.0;
...
if (VHS_FishEye_Hyperspace){
#line 770
const float prop =  float2(1024, 720).x /  float2(1024, 720).y;
const float2 m = float2(0.5, 0.5 / prop);
const float2 d = (uv* float2(1024, 720).xy) / float2(1024, 720).x - m;
...
else uv = (uv* float2(1024, 720).xy) / float2(1024, 720).x;
#line 788
uv.y *=  prop;
}
#line 796
...
if(SLN==0.0) SLN =  float2(1024, 720).y;
#line 855
if(SLN_Noise==0 || SLN_Noise>SLN) SLN_Noise = SLN;
#line 857
ONE_X = 1.0/ float2(1024, 720).x; 
ONE_Y = 1.0/ float2(1024, 720).y;
#line 861
if (VHS_Twitch_V){
...
#line 911
const float ScreenLinesNumX = SLN_Noise *  float2(1024, 720).x /  float2(1024, 720).y;
const float SLN_X = noiseQuantizeX*( float2(1024, 720).x - ScreenLinesNumX) + ScreenLinesNumX;
pn.x = floor( pn.x * SLN_X )/SLN_X;
#line 915
const float2 pn_ = pn* float2(1024, 720).xy;
#line 918
...
if(SLN==0.0) SLN =  float2(1024, 720).y;
#line 1099
float ONE_X = 1.0 /  float2(1024, 720).x;  
ONE_X *= bleedAmount; 
#line 1103
if (VHS_FishEye){
...
const float3 sums = 	 t2d( (p - float2( 0.5 *   	float2((1.0 / 1024), (1.0 / 720)).x, .0)) + float2( (offset-float(taps)) * (ONE_X), 0.0)); +
 t2d( (p - float2( 0.5 *   	float2((1.0 / 1024), (1.0 / 720)).x, .0)) + float2( (float(taps)-offset) * (ONE_X), 0.0)); ;
#line 1457
...
#line 1467
signal += t2d( (p - float2( 0.5 *   	float2((1.0 / 1024), (1.0 / 720)).x, .0))) * adj;
norm += adj;
signal = signal / norm;
} else {
#line 1472
...
const float one_x = 1.0/ float2(1024, 720).x;
...
p.y=1-p.y;
#line 1587
const float2 p_ = p* float2(1024, 720);
#line 1589
float ns = 0.0; 
float nt = 0.0; 
float nl = 0.0; 
...
if (VHS_FilmGrain) {
#line 1610
ns += filmGrain((p_-0.5* float2(1024, 720).xy)*0.5, t, filmGrainPower ) * filmGrainAmount;
}
#line 1613
