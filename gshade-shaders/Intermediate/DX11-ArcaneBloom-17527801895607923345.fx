...
namespace _ {
#line 42
 texture2D tArcaneBloom_Bloom0 { Width  = 2560 / 2; Height = 1440 / 2; Format = RGBA16F; };
 texture2D tArcaneBloom_Bloom1 { Width  = 2560 / 4; Height = 1440 / 4; Format = RGBA16F; };
 texture2D tArcaneBloom_Bloom2 { Width  = 2560 / 8; Height = 1440 / 8; Format = RGBA16F; };
 texture2D tArcaneBloom_Bloom3 { Width  = 2560 / 16; Height = 1440 / 16; Format = RGBA16F; };
 texture2D tArcaneBloom_Bloom4 { Width  = 2560 / 32; Height = 1440 / 32; Format = RGBA16F; };
#line 52
...
#line 358
 texture2D tArcaneBloom_Bloom0Alt { Width  = 2560 / 2; Height = 1440 / 2; Format = RGBA16F; }; sampler2D sBloom0Alt { Texture = tArcaneBloom_Bloom0Alt; };
 texture2D tArcaneBloom_Bloom1Alt { Width  = 2560 / 4; Height = 1440 / 4; Format = RGBA16F; }; sampler2D sBloom1Alt { Texture = tArcaneBloom_Bloom1Alt; };
 texture2D tArcaneBloom_Bloom2Alt { Width  = 2560 / 8; Height = 1440 / 8; Format = RGBA16F; }; sampler2D sBloom2Alt { Texture = tArcaneBloom_Bloom2Alt; };
 texture2D tArcaneBloom_Bloom3Alt { Width  = 2560 / 16; Height = 1440 / 16; Format = RGBA16F; }; sampler2D sBloom3Alt { Texture = tArcaneBloom_Bloom3Alt; };
 texture2D tArcaneBloom_Bloom4Alt { Width  = 2560 / 32; Height = 1440 / 32; Format = RGBA16F; }; sampler2D sBloom4Alt { Texture = tArcaneBloom_Bloom4Alt; };
#line 367
...
#line 552
  ...
  float4 PS_BlurX_Bloom0( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2((1.0 / 2560) * 2*0.5, 0.0); return float4(gaussian_blur(sBloom0, uv, dir), 1.0); }  float4 PS_BlurY_Bloom0Alt( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2(0.0, (1.0 / 1440) * 2*0.5); return float4(gaussian_blur(sBloom0Alt, uv, dir), 1.0); }
#line 555
  ...
  float4 PS_BlurX_Bloom1( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2((1.0 / 2560) * 4*1.0, 0.0); return float4(gaussian_blur(sBloom1, uv, dir), 1.0); }  float4 PS_BlurY_Bloom1Alt( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2(0.0, (1.0 / 1440) * 4*1.0); return float4(gaussian_blur(sBloom1Alt, uv, dir), 1.0); }
#line 558
  ...
  float4 PS_BlurX_Bloom2( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2((1.0 / 2560) * 8*1.0, 0.0); return float4(gaussian_blur(sBloom2, uv, dir), 1.0); }  float4 PS_BlurY_Bloom2Alt( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2(0.0, (1.0 / 1440) * 8*1.0); return float4(gaussian_blur(sBloom2Alt, uv, dir), 1.0); }
#line 561
...
#line 564
  ...
  float4 PS_BlurX_Bloom4( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2((1.0 / 2560) * 32*3.0, 0.0); return float4(gaussian_blur(sBloom4, uv, dir), 1.0); }  float4 PS_BlurY_Bloom4Alt( float4 position : SV_POSITION, float2 uv       : TEXCOORD ) : SV_TARGET { const float2 dir = float2(0.0, (1.0 / 1440) * 32*3.0); return float4(gaussian_blur(sBloom4Alt, uv, dir), 1.0); }
#line 570
