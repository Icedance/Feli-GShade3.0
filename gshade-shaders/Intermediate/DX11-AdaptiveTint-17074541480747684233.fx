...
#line 12
namespace Stats {
texture2D shared_texStats { Width = 1024; Height = 720; Format = RGBA8; MipLevels =   7.0; };
...
float fac = 0.0;
#line 422
float2 coord_pix = float2(1024, 720) * texcoord;
...
float valN = dot(tex2D(s, texcoord + float2(0.0, - float2((1.0 / 1024), (1.0 / 720)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valNE = dot(tex2D(s, texcoord + float2( float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valE = dot(tex2D(s, texcoord + float2( float2((1.0 / 1024), (1.0 / 720)).x, 0.0)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valSE = dot(tex2D(s, texcoord + float2( float2((1.0 / 1024), (1.0 / 720)).x,  float2((1.0 / 1024), (1.0 / 720)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valS = dot(tex2D(s, texcoord + float2(0.0,  float2((1.0 / 1024), (1.0 / 720)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valSW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 1024), (1.0 / 720)).x,  float2((1.0 / 1024), (1.0 / 720)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 1024), (1.0 / 720)).x, 0.0)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valNW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
#line 486
...
float depthN =  ReShade::GetLinearizedDepth(texcoord + float2(0.0, - float2((1.0 / 1024), (1.0 / 720)).y));
float depthNE = ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y));
float depthE =  ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 1024), (1.0 / 720)).x, 0.0));
float depthSE = ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 1024), (1.0 / 720)).x,  float2((1.0 / 1024), (1.0 / 720)).y));
float depthS =  ReShade::GetLinearizedDepth(texcoord + float2(0.0,  float2((1.0 / 1024), (1.0 / 720)).y));
float depthSW = ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 1024), (1.0 / 720)).x,  float2((1.0 / 1024), (1.0 / 720)).y));
float depthW =  ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 1024), (1.0 / 720)).x, 0.0));
float depthNW = ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y));
...
float fac = 0.0;
#line 26
float2 screencoord = float2(1024, 720) * texcoord;
...
> = 1.0;
#line 110
 uniform int2 AdaptiveTintDebugPosition < ui_type = "slider"; ui_category = "AdaptiveTintDebug"; ui_label = "Position"; ui_min = 0; ui_max = 1024; ui_step = 1; > = int2(0, 0); uniform float AdaptiveTintDebugOpacity < ui_type = "slider"; ui_category = "AdaptiveTintDebug"; ui_label = "Opacity"; ui_min = 0.0; ui_max = 1.0; ui_step = 0.01; > = 1.0; texture2D  texAdaptiveTintDebug { Width = 1024/4; Height = 720/4; Format = RGBA8; }; sampler2D  sAdaptiveTintDebug { Texture =  texAdaptiveTintDebug; }; float3  AdaptiveTintDebugoverlay(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target { float3 backbuffer = tex2D(ReShade::BackBuffer, texcoord).rgb; return Canvas::OverlaySampler(backbuffer,  sAdaptiveTintDebug, texcoord, AdaptiveTintDebugPosition, AdaptiveTintDebugOpacity); }
#line 115
texture2D texAlphaCheckerboard < source = "alpha-checkerboard.png"; > { Width = 1024; Height = 720; Format = RGBA8; };
