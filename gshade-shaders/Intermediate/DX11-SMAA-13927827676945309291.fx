#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\SMAA.fx"
#line 23
...
> = false;
#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\SMAA.fxh"
#line 600
float3 SMAAGatherNeighbours(float2 texcoord,
float4 offset[3],
 sampler tex) {
#line 604
return  tex2Dgather(tex, texcoord+float4((1.0/2560),(1.0/1377),2560,1377).xy*float2(-0.5,-0.5), 0).grb;
#line 611
}
#line 616
...
out float4 offset[3]) {
offset[0] = mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xyxy, float4(-1.0, 0.0, 0.0, -1.0), texcoord.xyxy);
offset[1] = mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xyxy, float4( 1.0, 0.0, 0.0,  1.0), texcoord.xyxy);
offset[2] = mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xyxy, float4(-2.0, 0.0, 0.0, -2.0), texcoord.xyxy);
}
#line 656
void SMAABlendingWeightCalculationVS(float2 texcoord,
out float2 pixcoord,
out float4 offset[3]) {
pixcoord = texcoord *  float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).zw;
#line 662
offset[0] = mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xyxy, float4(-0.25, -0.125,  1.25, -0.125), texcoord.xyxy);
offset[1] = mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xyxy, float4(-0.125, -0.25, -0.125,  1.25), texcoord.xyxy);
#line 666
offset[2] = mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xxyy,
...
out float4 offset) {
offset = mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xyxy, float4( 1.0, 0.0, 0.0,  1.0), texcoord.xyxy);
}
#line 690
...
const float3 t = float3( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xy, 1.0);
...
float4 coord = float4(texcoord, -1.0, 1.0);
coord.x += 0.25 *  float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).x; 
const float3 t = float3( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xy, 1.0);
while (coord.z < float( MaxSearchStepsDiagonal - 1) &&
coord.w > 0.9) {
coord.xyz = mad(t, float3(dir, 1.0), coord.xyz);
...
if (d.x + d.y > 2.0) { 
#line 1027
const float4 coords = mad(float4(-d.x + 0.25, d.x, d.y, -d.y - 0.25),  float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xyxy, texcoord.xyxy);
float4 c;
c.xy =  tex2Dlodoffset(edgesTex, float4(coords.xy, coords.xy), int2(-1,0)).rg;
c.zw =  tex2Dlodoffset(edgesTex, float4(coords.zw, coords.zw), int2(1,0)).rg;
...
if ( tex2Dlodoffset(edgesTex, float4(texcoord, texcoord), int2(1,0)).r > 0.0) {
...
if (d.x + d.y > 2.0) { 
#line 1062
const float4 coords = mad(float4(-d.x, -d.x, d.y, d.y),  float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xyxy, texcoord.xyxy);
float4 c;
c.x  =  tex2Dlodoffset(edgesTex, float4(coords.xy, coords.xy), int2(-1,0)).g;
c.y  =  tex2Dlodoffset(edgesTex, float4(coords.xy, coords.xy), int2(0,-1)).r;
c.zw =  tex2Dlodoffset(edgesTex, float4(coords.zw, coords.zw), int2(1,0)).gr;
float2 cc = mad(float2(2.0, 2.0), c.xz, c.yw);
#line 1070
SMAAMovc(bool2(step(0.9, d.zw)), cc, float2(0.0, 0.0));
#line 1073
...
texcoord = mad(-float2(2.0, 0.0),  float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xy, texcoord);
}
#line 1127
const float offset = mad(-(255.0 / 127.0), SMAASearchLength( searchTex, e, 0.0), 3.25);
return mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).x, offset, texcoord.x);
#line 1141
}
#line 1143
...
e.g == 0.0) { 
e =  tex2Dlod(edgesTex, float4(texcoord, texcoord)).rg;
texcoord = mad(-float2(0.0, 2.0),  float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xy, texcoord);
}
const float offset = mad(-(255.0 / 127.0), SMAASearchLength( searchTex, e.gr, 0.0), 3.25);
return mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).y, offset, texcoord.y);
}
#line 1167
...
factor.x -= rounding.x *  tex2Dlodoffset(edgesTex, float4(texcoord.xy, texcoord.xy), int2(0,1)).r;
factor.x -= rounding.y *  tex2Dlodoffset(edgesTex, float4(texcoord.zw, texcoord.zw), int2(1,1)).r;
factor.y -= rounding.x *  tex2Dlodoffset(edgesTex, float4(texcoord.xy, texcoord.xy), int2(0,-2)).r;
factor.y -= rounding.y *  tex2Dlodoffset(edgesTex, float4(texcoord.zw, texcoord.zw), int2(1,-2)).r;
#line 1213
...
#line 1224
float2 factor = float2(1.0, 1.0);
factor.x -= rounding.x *  tex2Dlodoffset(edgesTex, float4(texcoord.xy, texcoord.xy), int2(1,0)).g;
factor.x -= rounding.y *  tex2Dlodoffset(edgesTex, float4(texcoord.zw, texcoord.zw), int2(1,1)).g;
factor.y -= rounding.x *  tex2Dlodoffset(edgesTex, float4(texcoord.xy, texcoord.xy), int2(-2,0)).g;
factor.y -= rounding.y *  tex2Dlodoffset(edgesTex, float4(texcoord.zw, texcoord.zw), int2(-2,1)).g;
#line 1230
weights *= saturate(factor);
#line 1232
}
...
d.y = coords.z;
#line 1280
d = abs(round(mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).zz, d, -pixcoord.xx)));
#line 1284
const float2 sqrt_d = sqrt(d);
#line 1287
const float e2 =  tex2Dlodoffset(edgesTex, float4(coords.zy, coords.zy), int2(1,0)).r;
#line 1291
...
d.y = coords.z;
#line 1321
d = abs(round(mad( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).ww, d, -pixcoord.yy)));
#line 1325
const float2 sqrt_d = sqrt(d);
#line 1328
const float e2 =  tex2Dlodoffset(edgesTex, float4(coords.xz, coords.xz), int2(0,1)).g;
#line 1331
...
#line 1382
const float4 blendingCoord = mad(blendingOffset, float4( float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xy, - float4((1.0 / 2560), (1.0 / 1377), 2560, 1377).xy), texcoord.xyxy);
#line 1386
...
#line 127 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\SMAA.fx"
...
#line 128 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\SMAA.fx"
#line 132
