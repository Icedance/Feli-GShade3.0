...
return ori;
#line 116
const float depthTL = ReShade::GetLinearizedDepth( tex + float2(- float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y) ).r;
const float depthTR = ReShade::GetLinearizedDepth( tex + float2( float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y) ).r;
...
{
#line 160
blur_ori  = tex2D(ReShade::BackBuffer, tex + ( float2((1.0 / 1024), (1.0 / 720)) / 3.0) * offset_bias).rgb;  
blur_ori += tex2D(ReShade::BackBuffer, tex + (- float2((1.0 / 1024), (1.0 / 720)) / 3.0) * offset_bias).rgb; 
#line 163
blur_ori /= 2;  
#line 165
...
{
#line 176
blur_ori  = tex2D(ReShade::BackBuffer, tex + float2( float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y) * 0.5 * offset_bias).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex -  float2((1.0 / 1024), (1.0 / 720)) * 0.5 * offset_bias).rgb;  
blur_ori += tex2D(ReShade::BackBuffer, tex +  float2((1.0 / 1024), (1.0 / 720)) * 0.5 * offset_bias).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex - float2( float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y) * 0.5 * offset_bias).rgb; 
#line 181
blur_ori *= 0.25;  
...
{
#line 194
blur_ori  = tex2D(ReShade::BackBuffer, tex +  float2((1.0 / 1024), (1.0 / 720)) * float2(0.4, -1.2) * offset_bias).rgb;  
blur_ori += tex2D(ReShade::BackBuffer, tex -  float2((1.0 / 1024), (1.0 / 720)) * float2(1.2, 0.4) * offset_bias).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex +  float2((1.0 / 1024), (1.0 / 720)) * float2(1.2, 0.4) * offset_bias).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex -  float2((1.0 / 1024), (1.0 / 720)) * float2(0.4, -1.2) * offset_bias).rgb; 
#line 199
blur_ori *= 0.25;  
...
{
#line 212
blur_ori  = tex2D(ReShade::BackBuffer, tex + float2(0.5 *  float2((1.0 / 1024), (1.0 / 720)).x, - float2((1.0 / 1024), (1.0 / 720)).y * offset_bias)).rgb;  
blur_ori += tex2D(ReShade::BackBuffer, tex + float2(offset_bias * - float2((1.0 / 1024), (1.0 / 720)).x, 0.5 * - float2((1.0 / 1024), (1.0 / 720)).y)).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex + float2(offset_bias *  float2((1.0 / 1024), (1.0 / 720)).x, 0.5 *  float2((1.0 / 1024), (1.0 / 720)).y)).rgb; 
blur_ori += tex2D(ReShade::BackBuffer, tex + float2(0.5 * - float2((1.0 / 1024), (1.0 / 720)).x,  float2((1.0 / 1024), (1.0 / 720)).y * offset_bias)).rgb; 
#line 217
blur_ori /= 4.0;  
