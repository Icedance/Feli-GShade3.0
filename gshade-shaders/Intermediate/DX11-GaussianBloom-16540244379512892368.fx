...
for(int i = 1; i < 4; ++i)
{
blur += tex2D(GaussianBloomSampler, texcoord + float2(0.0, offset[i] *  float2((1.0 / 1920), (1.0 / 1080)).y)).rgb * weight[i];
blur += tex2D(GaussianBloomSampler, texcoord - float2(0.0, offset[i] *  float2((1.0 / 1920), (1.0 / 1080)).y)).rgb * weight[i];
}
}
#line 131
...
for(int i = 1; i < 6; ++i)
{
blur += tex2D(GaussianBloomSampler2, texcoord + float2( offset[i] *  float2((1.0 / 1920), (1.0 / 1080)).x,0.0)).rgb * weight[i];
blur += tex2D(GaussianBloomSampler2, texcoord - float2( offset[i] *  float2((1.0 / 1920), (1.0 / 1080)).x,0.0)).rgb * weight[i];
}
}
#line 214
