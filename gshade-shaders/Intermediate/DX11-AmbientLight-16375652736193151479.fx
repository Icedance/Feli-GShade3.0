#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\AmbientLight.fx"
#line 33
...
#line 107 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\AmbientLight.fx"
#line 129
uniform float2 AL_t < source = "pingpong"; min = 0.0f; max = 6.28f; step = float2(0.1f, 0.2f); >;
#line 133
texture alInTex  { Width = 2560 / 16; Height = 1440 / 16; Format = RGBA32F; };
texture alOutTex { Width = 2560 / 16; Height = 1440 / 16; Format = RGBA32F; };
...
texture lensDBTex  < source = "LayerA.png";  > { Width =  1920; Height =  1080; MipLevels = 1; Format = RGBA8; };
texture lensDB2Tex < source = "LensDB2_Feli.png"; > { Width =  1920; Height =  1080; MipLevels = 1; Format = RGBA8; };
...
#line 248
[flatten]
if ((texcoord.x + sampleOffsets[1] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x) < 1.05)
hgb += tex2D(alInColor, texcoord + float2(sampleOffsets[1] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x, 0.0)) * sampleWeights[1] * stepMult;
[flatten]
if ((texcoord.x - sampleOffsets[1] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x) > -0.05)
hgb += tex2D(alInColor, texcoord - float2(sampleOffsets[1] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x, 0.0)) * sampleWeights[1] * stepMult;
#line 255
[flatten]
if ((texcoord.x + sampleOffsets[2] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x) < 1.05)
hgb += tex2D(alInColor, texcoord + float2(sampleOffsets[2] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x, 0.0)) * sampleWeights[2] * stepMult;
[flatten]
if ((texcoord.x - sampleOffsets[2] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x) > -0.05)
hgb += tex2D(alInColor, texcoord - float2(sampleOffsets[2] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x, 0.0)) * sampleWeights[2] * stepMult;
#line 262
[flatten]
if ((texcoord.x + sampleOffsets[3] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x) < 1.05)
hgb += tex2D(alInColor, texcoord + float2(sampleOffsets[3] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x, 0.0)) * sampleWeights[3] * stepMult;
[flatten]
if ((texcoord.x - sampleOffsets[3] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x) > -0.05)
hgb += tex2D(alInColor, texcoord - float2(sampleOffsets[3] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x, 0.0)) * sampleWeights[3] * stepMult;
#line 269
[flatten]
if ((texcoord.x + sampleOffsets[4] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x) < 1.05)
hgb += tex2D(alInColor, texcoord + float2(sampleOffsets[4] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x, 0.0)) * sampleWeights[4] * stepMult;
[flatten]
if ((texcoord.x - sampleOffsets[4] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x) > -0.05)
hgb += tex2D(alInColor, texcoord - float2(sampleOffsets[4] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).x, 0.0)) * sampleWeights[4] * stepMult;
}
...
#line 285
[flatten]
if ((texcoord.y + sampleOffsets[1] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y) < 1.05)
vgb += tex2D(alOutColor, texcoord + float2(0.0, sampleOffsets[1] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y)) * sampleWeights[1] * stepMult;
[flatten]
if ((texcoord.y - sampleOffsets[1] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y) > -0.05)
vgb += tex2D(alOutColor, texcoord - float2(0.0, sampleOffsets[1] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y)) * sampleWeights[1] * stepMult;
#line 292
[flatten]
if ((texcoord.y + sampleOffsets[2] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y) < 1.05)
vgb += tex2D(alOutColor, texcoord + float2(0.0, sampleOffsets[2] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y)) * sampleWeights[2] * stepMult;
[flatten]
if ((texcoord.y - sampleOffsets[2] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y) > -0.05)
vgb += tex2D(alOutColor, texcoord - float2(0.0, sampleOffsets[2] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y)) * sampleWeights[2] * stepMult;
#line 299
[flatten]
if ((texcoord.y + sampleOffsets[3] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y) < 1.05)
vgb += tex2D(alOutColor, texcoord + float2(0.0, sampleOffsets[3] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y)) * sampleWeights[3] * stepMult;
[flatten]
if ((texcoord.y - sampleOffsets[3] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y) > -0.05)
vgb += tex2D(alOutColor, texcoord - float2(0.0, sampleOffsets[3] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y)) * sampleWeights[3] * stepMult;
#line 306
[flatten]
if ((texcoord.y + sampleOffsets[4] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y) < 1.05)
vgb += tex2D(alOutColor, texcoord + float2(0.0, sampleOffsets[4] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y)) * sampleWeights[4] * stepMult;
[flatten]
if ((texcoord.y - sampleOffsets[4] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y) > -0.05)
vgb += tex2D(alOutColor, texcoord - float2(0.0, sampleOffsets[4] *  float2(1.0f / (2560 / 16.0f), 1.0f / (1440 / 16.0f)).y)) * sampleWeights[4] * stepMult;
}
#line 314
...
dither = lerp(2.0 * dither, -2.0 * dither, frac(dot(texcoord,  float2(2560, 1440) * float2(1.0 / 16.0, 10.0 / 36.0)) + 0.25));
}
#line 454
if (all(base.xyz == 1.0))
{
