#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Glitch.fx"
...
{
return tex2D(samp, uv - fmod(uv, float2(pixelSize,pixelSize) /  float2(2560, 1377)));
}
#line 68
float rand(float n)
{
...
{
const float dx = sampleSize * (1.0 / 2560);
const float dy = sampleSize * (1.0 / 1377);
return (
...
{
const float2 pixel =  float2((1.0 / 2560), (1.0 / 1377));
const float3 field = rgb2hsv(edge(samp, uv, edgeSize));
const float2 distort = pixel * sin((field.rb) *  3.14159265 * 2.0);
const float shiftx = noise(float2(quantize(uv.y + 31.5, 1377 /  16.0) * Timer*0.001, frac(Timer*0.001) * 300.0));
const float shifty = noise(float2(quantize(uv.x + 11.5, 2560 /  16.0) * Timer*0.001, frac(Timer*0.001) * 100.0));
...
float Amount;
#line 126
const float2 texcoord = fragCoord *  float2(2560, 1377); 
const float2 uv = texcoord.xy /  float2(2560, 1377);
if (bUseUV) {
Amount = uv.x; 
}
