...
{
return tex2D(samp, uv - fmod(uv, float2(pixelSize,pixelSize) /  float2(1024, 720)));
}
#line 68
float rand(float n)
{
...
{
const float2 pixel =  float2((1.0 / 1024), (1.0 / 720));
...
float Amount;
#line 126
const float2 texcoord = fragCoord *  float2(1024, 720); 
const float2 uv = texcoord.xy /  float2(1024, 720);
if (bUseUV) {
Amount = uv.x; 
}
