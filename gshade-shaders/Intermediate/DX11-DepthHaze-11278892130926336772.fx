...
#line 40
texture   Otis_FragmentBuffer1 	{ Width = 1024; Height = 720; Format = RGBA8;};
texture   Otis_FragmentBuffer2 	< pooled = true; > { Width = 1024; Height = 720; Format = RGBA8;};
#line 46
...
for(float i = 1; i < 5; ++i)
{
float2 sourceCoords = texcoord + float2(i *  float2((1.0 / 1024), (1.0 / 720)).x, 0.0);
...
n+=weight;
#line 71
sourceCoords = texcoord - float2(i *  float2((1.0 / 1024), (1.0 / 720)).x, 0.0);
...
for(float j = 1; j < 5; ++j)
{
float2 sourceCoords = texcoord + float2(0.0, j *  float2((1.0 / 1024), (1.0 / 720)).y);
...
n+=weight;
#line 93
sourceCoords = texcoord - float2(0.0, j *  float2((1.0 / 1024), (1.0 / 720)).y);
