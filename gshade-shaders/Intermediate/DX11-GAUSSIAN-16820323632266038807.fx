#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\GAUSSIAN.fx"
...
#line 9 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\GAUSSIAN.fx"
#line 36
...
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(sampleOffsets[i]*gBloomHW *  float2((1.0 / 2560),(1.0 / 1377)).x, 0.0), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(sampleOffsets[i]*gBloomHW *  float2((1.0 / 2560),(1.0 / 1377)).x, 0.0), 0.0, 0.0)) * sampleWeights[i];
}
return color;
}
...
float4 color = tex2D(ReShade::BackBuffer, texcoord) * sampleWeights[0];
for(int i = 1; i < gN_PASSES; ++i) {
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(0.0, sampleOffsets[i]*gBloomVW *  float2((1.0 / 2560),(1.0 / 1377)).y), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(0.0, sampleOffsets[i]*gBloomVW *  float2((1.0 / 2560),(1.0 / 1377)).y), 0.0, 0.0)) * sampleWeights[i];
}
return color;
}
...
float4 color = tex2D(ReShade::BackBuffer, texcoord) * sampleWeights[0];
for(int i = 1; i < gN_PASSES; ++i) {
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(sampleOffsets[i]*gBloomSW *  float2((1.0 / 2560),(1.0 / 1377)).x, sampleOffsets[i] *  float2((1.0 / 2560),(1.0 / 1377)).y), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(sampleOffsets[i]*gBloomSW *  float2((1.0 / 2560),(1.0 / 1377)).x, sampleOffsets[i] *  float2((1.0 / 2560),(1.0 / 1377)).y), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(-sampleOffsets[i]*gBloomSW *  float2((1.0 / 2560),(1.0 / 1377)).x, sampleOffsets[i] *  float2((1.0 / 2560),(1.0 / 1377)).y), 0.0, 0.0)) * sampleWeights[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(sampleOffsets[i]*gBloomSW *  float2((1.0 / 2560),(1.0 / 1377)).x, -sampleOffsets[i] *  float2((1.0 / 2560),(1.0 / 1377)).y), 0.0, 0.0)) * sampleWeights[i];
}
