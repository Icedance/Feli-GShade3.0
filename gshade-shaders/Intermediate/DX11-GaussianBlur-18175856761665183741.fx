...
#line 27
texture GaussianBlurTex < pooled = true; > { Width = 2560; Height = 1440; Format = RGBA8; };
...
for(int i = 1; i < 4; ++i)
{
color += tex2D(GaussianBlurSampler, texcoord + float2(0.0, offset[i] *  float2((1.0 / 2560), (1.0 / 1440)).y) * GaussianBlurOffset).rgb * weight[i];
color += tex2D(GaussianBlurSampler, texcoord - float2(0.0, offset[i] *  float2((1.0 / 2560), (1.0 / 1440)).y) * GaussianBlurOffset).rgb * weight[i];
}
}
#line 50
...
for(int i = 1; i < 4; ++i)
{
color += tex2D(ReShade::BackBuffer, texcoord + float2(offset[i] *  float2((1.0 / 2560), (1.0 / 1440)).x, 0.0) * GaussianBlurOffset).rgb * weight[i];
color += tex2D(ReShade::BackBuffer, texcoord - float2(offset[i] *  float2((1.0 / 2560), (1.0 / 1440)).x, 0.0) * GaussianBlurOffset).rgb * weight[i];
}
}
#line 136
