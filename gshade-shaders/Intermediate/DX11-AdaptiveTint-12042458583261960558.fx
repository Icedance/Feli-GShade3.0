#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\AdaptiveTint.fx"
...
#line 7 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\AdaptiveTint.fx"
#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Stats.fxh"
#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\ReShade.fxh"
#line 6 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Stats.fxh"
#line 12
namespace Stats {
texture2D shared_texStats { Width = 2560; Height = 1377; Format = RGBA8; MipLevels =   7.0; };
...
#line 8 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\AdaptiveTint.fx"
#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Tools.fxh"
#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\ReShade.fxh"
#line 6 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Tools.fxh"
#line 84
struct sctpoint {
float3 color;
float2 coord;
...
float fac = 0.0;
#line 422
float2 coord_pix = float2(2560, 1377) * texcoord;
...
float valN = dot(tex2D(s, texcoord + float2(0.0, - float2((1.0 / 2560), (1.0 / 1377)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valNE = dot(tex2D(s, texcoord + float2( float2((1.0 / 2560), (1.0 / 1377)).x, - float2((1.0 / 2560), (1.0 / 1377)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valE = dot(tex2D(s, texcoord + float2( float2((1.0 / 2560), (1.0 / 1377)).x, 0.0)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valSE = dot(tex2D(s, texcoord + float2( float2((1.0 / 2560), (1.0 / 1377)).x,  float2((1.0 / 2560), (1.0 / 1377)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valS = dot(tex2D(s, texcoord + float2(0.0,  float2((1.0 / 2560), (1.0 / 1377)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valSW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 2560), (1.0 / 1377)).x,  float2((1.0 / 2560), (1.0 / 1377)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 2560), (1.0 / 1377)).x, 0.0)).rgb,  float3(0.2126, 0.7151, 0.0721));
float valNW = dot(tex2D(s, texcoord + float2(- float2((1.0 / 2560), (1.0 / 1377)).x, - float2((1.0 / 2560), (1.0 / 1377)).y)).rgb,  float3(0.2126, 0.7151, 0.0721));
#line 486
...
float depthN =  ReShade::GetLinearizedDepth(texcoord + float2(0.0, - float2((1.0 / 2560), (1.0 / 1377)).y));
float depthNE = ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 2560), (1.0 / 1377)).x, - float2((1.0 / 2560), (1.0 / 1377)).y));
float depthE =  ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 2560), (1.0 / 1377)).x, 0.0));
float depthSE = ReShade::GetLinearizedDepth(texcoord + float2( float2((1.0 / 2560), (1.0 / 1377)).x,  float2((1.0 / 2560), (1.0 / 1377)).y));
float depthS =  ReShade::GetLinearizedDepth(texcoord + float2(0.0,  float2((1.0 / 2560), (1.0 / 1377)).y));
float depthSW = ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 2560), (1.0 / 1377)).x,  float2((1.0 / 2560), (1.0 / 1377)).y));
float depthW =  ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 2560), (1.0 / 1377)).x, 0.0));
float depthNW = ReShade::GetLinearizedDepth(texcoord + float2(- float2((1.0 / 2560), (1.0 / 1377)).x, - float2((1.0 / 2560), (1.0 / 1377)).y));
...
#line 9 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\AdaptiveTint.fx"
#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Canvas.fxh"
#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\ReShade.fxh"
#line 6 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Canvas.fxh"
#line 20
namespace Canvas {
...
float fac = 0.0;
#line 26
float2 screencoord = float2(2560, 1377) * texcoord;
float2 overlay_size = (float2)tex2Dsize(overlay, 0);
offset.x = clamp(offset.x, 0, 2560 - overlay_size.x);
offset.y = clamp(offset.y, 0, 1377 - overlay_size.y);
float2 border_min = (float2)offset;
...
#line 10 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\AdaptiveTint.fx"
#line 22
uniform int iUIInfo<
ui_type = "combo";
ui_label = "Info";
...
> = 1.0;
#line 110
 uniform int2 AdaptiveTintDebugPosition < ui_type = "slider"; ui_category = "AdaptiveTintDebug"; ui_label = "Position"; ui_min = 0; ui_max = 2560; ui_step = 1; > = int2(0, 0); uniform float AdaptiveTintDebugOpacity < ui_type = "slider"; ui_category = "AdaptiveTintDebug"; ui_label = "Opacity"; ui_min = 0.0; ui_max = 1.0; ui_step = 0.01; > = 1.0; texture2D  texAdaptiveTintDebug { Width = 2560/4; Height = 1377/4; Format = RGBA8; }; sampler2D  sAdaptiveTintDebug { Texture =  texAdaptiveTintDebug; }; float3  AdaptiveTintDebugoverlay(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target { float3 backbuffer = tex2D(ReShade::BackBuffer, texcoord).rgb; return Canvas::OverlaySampler(backbuffer,  sAdaptiveTintDebug, texcoord, AdaptiveTintDebugPosition, AdaptiveTintDebugOpacity); }
#line 115
texture2D texAlphaCheckerboard < source = "alpha-checkerboard.png"; > { Width = 2560; Height = 1377; Format = RGBA8; };
...
#line 196
  ...
  AdaptiveTintDebug = Canvas::DrawScale(AdaptiveTintDebug, float3(1.0,0.0,0.0), float3(0.0,0.0,1.0), int2(0,10), int2(10,1377/4-10), GetColorTemp(texcoord), float3(0.0,0.0,0.0), float2(texcoord.x,  1.0 - texcoord.y),  sAdaptiveTintDebug, 0.002);;
  AdaptiveTintDebug = Canvas::DrawScale(AdaptiveTintDebug, float3(0.0,0.0,0.0), float3(1.0,1.0,1.0), int2(10,0), int2(2560/4-10,10), avgLuma, float3(1.0,0.0,1.0), float2(texcoord.x,  1.0 - texcoord.y),  sAdaptiveTintDebug, 0.002);;
