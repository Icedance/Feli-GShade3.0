#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Pirate_Bloom.fx"
...
#line 2 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\Pirate_Bloom.fx"
#line 11
...
> = false;
#line 44
texture		TexBloomH { Width = 2560* 	0.25			; Height = 1377* 	0.25			; Format = RGBA8;};
texture		TexBloomV { Width = 2560* 	0.25			; Height = 1377* 	0.25			; Format = RGBA8;};
...
for(int i=1; i < 	12			; i++)
{
ret += max(tex2D(ReShade::BackBuffer, coords + float2(i *   	float2((1.0 / 2560), (1.0 / 1377)).x * BLOOM_RADIUS, 0.0)) - BLOOM_THRESHOLD, 0.0);
ret += max(tex2D(ReShade::BackBuffer, coords - float2(i *   	float2((1.0 / 2560), (1.0 / 1377)).x * BLOOM_RADIUS, 0.0)) - BLOOM_THRESHOLD, 0.0);
}
#line 67
...
for(int i=1; i < 	12			; i++)
{
ret += tex2D(SamplerBloomV, coords + float2(i *   	float2((1.0 / 2560), (1.0 / 1377)).x * BLOOM_RADIUS, 0.0));
ret += tex2D(SamplerBloomV, coords - float2(i *   	float2((1.0 / 2560), (1.0 / 1377)).x * BLOOM_RADIUS, 0.0));
}
#line 79
return ret / 	((	12			 * 2) - 1);
...
for(int i=1; i < 	12			; i++)
{
ret += tex2D(SamplerBloomH, coords + float2(0.0, i *   	float2((1.0 / 2560), (1.0 / 1377)).y * BLOOM_RADIUS));
ret += tex2D(SamplerBloomH, coords - float2(0.0, i *   	float2((1.0 / 2560), (1.0 / 1377)).y * BLOOM_RADIUS));
}
#line 91
return ret / 	((	12			 * 2) - 1);
