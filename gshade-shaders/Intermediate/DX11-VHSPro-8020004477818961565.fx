#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\VHSPro.fx"
...
#line 9 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\VHSPro.fx"
#line 11
...
#line 447
texture2D VHS_InputTexA    { Width = 2560; Height = 1440; Format = RGBA8; };
texture2D VHS_InputTexB    { Width = 2560; Height = 1440; Format = RGBA8; };
texture2D VHS_FeedbackTexA { Width = 2560; Height = 1440; Format = RGBA8; };
texture2D VHS_FeedbackTexB { Width = 2560; Height = 1440; Format = RGBA8; };
texture2D _TapeTex { Width =  	int(((float)   int( 240.0	)*(float)2560/(float)1440)); Height =    int( 240.0	); Format = RGBA8; };
#line 453
...
#line 561
float scans = 0.5*(cos( (p.y*screenLinesNum+t_sl)*2.0* 3.14159265359) + 1.0);
...
{
return (cos( uv.y *  3.14159265359 * 2.0 * s + p)+1.0)*0.5;
}
#line 575
float2 stretch(float2 uv, float t, float mw, float wcs, float lfs, float lfp){
#line 577
const float SLN = screenLinesNum; 
#line 579
const float tt = t*wcs; 
const float t2 = tt-fmod(tt, 0.5);
#line 583
float w = gcos(uv, 2.0*(1.0-frac(t2)),  3.14159265359-t2) * clamp( gcos(uv, frac(t2), t2) , 0.5, 1.0);
#line 585
w = floor(w*mw)/mw;
w *= mw;
#line 588
...
#line 707
const float y = p.y* float2(2560, 1440).y;
const float s = t*2.0;
...
if (VHS_FishEye_Hyperspace){
#line 770
const float prop =  float2(2560, 1440).x /  float2(2560, 1440).y;
const float2 m = float2(0.5, 0.5 / prop);
const float2 d = (uv* float2(2560, 1440).xy) / float2(2560, 1440).x - m;
...
else uv = (uv* float2(2560, 1440).xy) / float2(2560, 1440).x;
#line 788
uv.y *=  prop;
}
#line 796
...
float2 p = txcoord.xy;
#line 844
float SLN = screenLinesNum; 
float SLN_Noise = noiseLinesNum; 
float ONE_X = 0.0;
float ONE_Y = 0.0;
#line 851
SLN = screenLinesNum; 
SLN_Noise = noiseLinesNum; 
if(SLN==0.0) SLN =  float2(2560, 1440).y;
#line 855
if(SLN_Noise==0 || SLN_Noise>SLN) SLN_Noise = SLN;
#line 857
ONE_X = 1.0/ float2(2560, 1440).x; 
ONE_Y = 1.0/ float2(2560, 1440).y;
#line 861
if (VHS_Twitch_V){
...
#line 911
const float ScreenLinesNumX = SLN_Noise *  float2(2560, 1440).x /  float2(2560, 1440).y;
const float SLN_X = noiseQuantizeX*( float2(2560, 1440).x - ScreenLinesNumX) + ScreenLinesNumX;
pn.x = floor( pn.x * SLN_X )/SLN_X;
#line 915
const float2 pn_ = pn* float2(2560, 1440).xy;
#line 918
...
if(tnl>0.55) {
#line 938
const float sh = sin( 1.0* 3.14159265359*(float(ii)/float(20))) ; 
...
if(SLN==0.0) SLN =  float2(2560, 1440).y;
#line 1099
float ONE_X = 1.0 /  float2(2560, 1440).x;  
ONE_X *= bleedAmount; 
#line 1103
if (VHS_FishEye){
...
const float3 sums = 	 t2d( (p - float2( 0.5 *   	float2((1.0 / 2560), (1.0 / 1440)).x, .0)) + float2( (offset-float(taps)) * (ONE_X), 0.0)); +
 t2d( (p - float2( 0.5 *   	float2((1.0 / 2560), (1.0 / 1440)).x, .0)) + float2( (float(taps)-offset) * (ONE_X), 0.0)); ;
#line 1457
...
#line 1467
signal += t2d( (p - float2( 0.5 *   	float2((1.0 / 2560), (1.0 / 1440)).x, .0))) * adj;
norm += adj;
signal = signal / norm;
} else {
#line 1472
...
const float one_x = 1.0/ float2(2560, 1440).x;
...
p.y=1-p.y;
#line 1587
const float2 p_ = p* float2(2560, 1440);
#line 1589
float ns = 0.0; 
float nt = 0.0; 
float nl = 0.0; 
...
if (VHS_FilmGrain) {
#line 1610
ns += filmGrain((p_-0.5* float2(2560, 1440).xy)*0.5, t, filmGrainPower ) * filmGrainAmount;
}
#line 1613
