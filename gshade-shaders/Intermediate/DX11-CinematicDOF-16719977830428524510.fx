...
texture texCDCoC				{ Width = 1024; Height = 720; Format = R16F; };
texture texCDCoCTileTmp			{ Width = 1024/(			1			*		1); Height = 720/(			1			*		1); Format = R16F; };	
texture texCDCoCTile			{ Width = 1024/(			1			*		1); Height = 720/(			1			*		1); Format = R16F; };	
texture texCDCoCTileNeighbor	{ Width = 1024/(			1			*		1); Height = 720/(			1			*		1); Format = R16F; };	
texture texCDCoCTmp1			{ Width = 1024/2; Height = 720/2; Format = R16F; };	
texture texCDCoCBlurred			{ Width = 1024/2; Height = 720/2; Format = RG16F; };	
texture texCDBuffer1 			{ Width = 1024/2; Height = 720/2; Format = RGBA8; };
texture texCDBuffer2 			{ Width = 1024/2; Height = 720/2; Format = RGBA8; };
texture texCDBuffer3 			{ Width = 1024/2; Height = 720/2; Format = RGBA8; }; 
texture texCDBuffer4 			{ Width = 1024; Height = 720; Format = RGBA8; }; 	
texture texCDBuffer5 			{ Width = 1024; Height = 720; Format = RGBA8; }; 	
#line 365
...
{
const float tileSize = 			1			 * (float(1024) / float(	1920.0f));
float minCoC = 10;
float coc;
float2 coordOffset = float2( float2((1.0 / 1024), (1.0 / 720)).x, 0);
...
coc = tex2Dlod(source, float4(texcoord - coordOffset, 0, 0)).r;
minCoC = min(minCoC, coc);
coordOffset.x+= float2((1.0 / 1024), (1.0 / 720)).x;
}
return minCoC;
}
#line 472
...
{
float tileSize = 			1			 * (float( float2(1024, 720).y) / float(	1200.0f));
float minCoC = 10;
float coc;
float2 coordOffset = float2(0,  float2((1.0 / 1024), (1.0 / 720)).y);
for(float i = 0; i <= tileSize; ++i)
{
coc = tex2Dlod(source, float4(texcoord + coordOffset, 0, 0)).r;
minCoC = min(minCoC, coc);
coc = tex2Dlod(source, float4(texcoord - coordOffset, 0, 0)).r;
minCoC = min(minCoC, coc);
coordOffset.y+= float2((1.0 / 1024), (1.0 / 720)).y;
}
return minCoC;
}
#line 490
...
{
float minCoC = 10;
const float tileSizeX = 			1			 * (float( float2(1024, 720).x) / float(	1920.0f));
const float tileSizeY = 			1			 * (float( float2(1024, 720).y) / float(	1200.0f));
#line 497
const float2 baseCoordOffset = float2( float2((1.0 / 1024), (1.0 / 720)).x * (tileSizeX*2+1),  float2((1.0 / 1024), (1.0 / 720)).x * (tileSizeY*2+1));
for(float i=-1;i<2;i++)
{
for(float j=-1;j<2;j++)
{
...
{
float4 toReturn = (radius/2 <= length( float2((1.0 / 1024), (1.0 / 720)))) && showInFocus ? float4(0.0, 0.0, 1.0, 1.0) : float4(radius, radius, radius, 1.0);
if(radius < 0)
{
...
const float2 ringRadiusDeltaCoords =  float2((1.0 / 1024), (1.0 / 720)) * (nearPlaneBlurInPixels / (numberOfRings-1));
...
float3 averageGained = AccentuateWhites(average.rgb);
float2 pointOffset = float2(0,0);
const float2 ringRadiusDeltaCoords =  ( float2((1.0 / 1024), (1.0 / 720)) * blurInfo.farPlaneMaxBlurInPixels * fragmentRadius) / blurInfo.numberOfRings;
float2 currentRingRadiusCoords = ringRadiusDeltaCoords;
...
float2 pointOffset = float2(0,0);
#line 736
const float2 ringRadiusDeltaCoords =  float2((1.0 / 1024), (1.0 / 720))
...
const float lengthPixelSize = length( float2((1.0 / 1024), (1.0 / 720)));
...
toFill.pixelSizeLength = length( float2((1.0 / 1024), (1.0 / 720)));
#line 870
...
float pixelSizeLength = length( float2((1.0 / 1024), (1.0 / 720)));
...
blurInfo.cocFactorPerPixel = length( float2((1.0 / 1024), (1.0 / 720))) * blurInfo.farPlaneMaxBlurInPixels;	
return blurInfo;
}
#line 930
void PS_DetermineCurrentFocus(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float fragment : SV_Target0)
{
const float2 autoFocusPointToUse = UseMouseDrivenAutoFocus ? MouseCoords *  float2((1.0 / 1024), (1.0 / 720)) : AutoFocusPoint;
...
float2( float2((1.0 / 1024), (1.0 / 720)).x * ( float2(1024, 720).x/	1920.0f), 0.0), true);
}
#line 999
...
float2(0.0,  float2((1.0 / 1024), (1.0 / 720)).y * ( float2(1024, 720).y/	1200.0f)), false),
...
{
const float4 coord =  float2((1.0 / 1024), (1.0 / 720)).xyxy * float4(1, 1, -1, 0);
float4 average;
...
{
fragment = PerformFullFragmentGaussianBlur(SamplerCDBuffer4, texcoord, float2( float2((1.0 / 1024), (1.0 / 720)).x, 0.0));
}
#line 1053
...
#line 1082
fragment = PerformFullFragmentGaussianBlur(SamplerCDBuffer5, focusInfo.texcoord, float2(0.0,  float2((1.0 / 1024), (1.0 / 720)).y));
...
fragment.rgb = lerp(originalFragment.rgb, fragment.rgb, saturate(coc < length( float2((1.0 / 1024), (1.0 / 720))) ? 0 : 4 * coc));
fragment.w = 1.0;
#line 1091
...
if(UseAutoFocus)
{
const float2 focusPointCoords = UseMouseDrivenAutoFocus ? MouseCoords *  float2((1.0 / 1024), (1.0 / 720)) : AutoFocusPoint;
