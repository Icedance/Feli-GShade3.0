...
#line 27
texture GaussianBlurTex < pooled = true; > { Width = 1024; Height = 720; Format = RGBA8; };
...
for(int i = 1; i < 4; ++i)
{
color += tex2D(GaussianBlurSampler, texcoord + float2(0.0, offset[i] *  float2((1.0 / 1024), (1.0 / 720)).y) * GaussianBlurOffset).rgb * weight[i];
color += tex2D(GaussianBlurSampler, texcoord - float2(0.0, offset[i] *  float2((1.0 / 1024), (1.0 / 720)).y) * GaussianBlurOffset).rgb * weight[i];
}
}
#line 50
...
for(int i = 1; i < 4; ++i)
{
color += tex2D(ReShade::BackBuffer, texcoord + float2(offset[i] *  float2((1.0 / 1024), (1.0 / 720)).x, 0.0) * GaussianBlurOffset).rgb * weight[i];
color += tex2D(ReShade::BackBuffer, texcoord - float2(offset[i] *  float2((1.0 / 1024), (1.0 / 720)).x, 0.0) * GaussianBlurOffset).rgb * weight[i];
}
}
#line 136
