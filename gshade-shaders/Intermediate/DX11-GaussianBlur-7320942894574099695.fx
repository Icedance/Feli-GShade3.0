#line 1 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\GaussianBlur.fx"
#line 7
...
#line 25 "C:\SquareEnix\FINAL FANTASY XIV - A Realm Reborn\game\gshade-shaders\Shaders\GaussianBlur.fx"
#line 27
texture GaussianBlurTex < pooled = true; > { Width = 2560; Height = 1377; Format = RGBA8; };
...
for(int i = 1; i < 4; ++i)
{
color += tex2D(GaussianBlurSampler, texcoord + float2(0.0, offset[i] *  float2((1.0 / 2560), (1.0 / 1377)).y) * GaussianBlurOffset).rgb * weight[i];
color += tex2D(GaussianBlurSampler, texcoord - float2(0.0, offset[i] *  float2((1.0 / 2560), (1.0 / 1377)).y) * GaussianBlurOffset).rgb * weight[i];
}
}
#line 50
...
for(int i = 1; i < 4; ++i)
{
color += tex2D(ReShade::BackBuffer, texcoord + float2(offset[i] *  float2((1.0 / 2560), (1.0 / 1377)).x, 0.0) * GaussianBlurOffset).rgb * weight[i];
color += tex2D(ReShade::BackBuffer, texcoord - float2(offset[i] *  float2((1.0 / 2560), (1.0 / 1377)).x, 0.0) * GaussianBlurOffset).rgb * weight[i];
}
}
#line 136
